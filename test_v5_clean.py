...
u1 = r.json()
print(f'首次催办: ID={u1["id"]} 次数={u1["urge_count"]}')
...
u2 = r.json()
print(f'重复催办: ID={u2["id"]} 次数={u2["urge_count"]}')
...
u3 = r.json()
print(f'不同目标催办: ID={u3["id"]} 次数={u3["urge_count"]}')
...
print(f'时间线节点完整: {all_types} OK')
pending = [n for n in timeline['nodes'] if n['status'] == 'pending']
print(f'待处理节点: {[n["node_type"] for n in pending]}')
...
print(f'分组数: {len(review["groups"])}')
for g in review['groups']:
    print(f'  区域={g["district"]} 预警数={g["warning_count"]}')
for pf in review['performance']:
    print(f'  绩效: 区域={pf["district"]} 工单完成率={pf["work_order_completion_rate"]} 超时={pf["timeout_count"]}')
